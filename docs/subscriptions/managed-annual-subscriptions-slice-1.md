...
- Необязательный Viva `productId` как единственный `PRODUCT_CANDIDATE` данной версии правил. Сервер возвращает `evidenceState=UNVERIFIED`; кандидат не используется runtime-контуром.
...
6. Указать Viva `productId`; в read-back получить тот же ID с `referenceKind=PRODUCT_CANDIDATE` и серверным `evidenceState=UNVERIFIED`.
7. Убедиться, что скидка на игру выключена, `benefitRules=[]`, а сетевой журнал не содержит запроса к Viva.
8. Создать вторую версию с другим окном или Viva `productId`; получить версии `1` и `2`, первая не изменена.
9. Создать лестницу: `50 × 19 800`, `50 × 23 800`, `50 × 36 000`, `50 × 48 000` рублей.
10. Проверить minor units: `1 980 000`, `2 380 000`, `3 600 000`, `4 800 000`; все counters нулевые.
11. Создать отдельную фазу `100` единиц, `7` в день, `09:00 Europe/Moscow`; она остаётся DRAFT и ничего не выкладывает.
...
7. Пустой Viva `externalId`, неизвестный `referenceKind`, переданный клиентом `evidenceState` и любой `clientSubscriptionId` внутри policy должны вернуть validation error без записи в БД.
8. Повтор старого `Idempotency-Key` с добавленным или изменённым `providerBinding` должен вернуть `409 IDEMPOTENCY_CONFLICT`.
...
Расширенная DRAFT-модель жизненного цикла, заморозки, отмен, продления, владения, удержания и аналитики описана в `managed-annual-subscriptions-model-v2.md`. Эти поля являются настройками-кандидатами и не включают runtime-механику.
