...
- `LK_PADELHUB_STAFF_API_BASE_URL=https://...` (внутренний LK endpoint durable удаления игрока; доступен только backend-у ЦУП)
- `LK_PADELHUB_STAFF_INTEGRATION_TOKEN=<token>` (Bearer-токен только для server-to-server вызова; не передавать в браузер)
- `LK_PADELHUB_STAFF_REQUEST_TIMEOUT_MS=8000` (опционально, timeout вызова LK; 1000–30000 мс)
...
- `VIVA_TOURNAMENT_EXERCISE_TYPE_IDS=839,1013` (опционально; какие `exerciseTypeIds` считать турнирами)
  - Дополнительно ЦУП импортирует точную связку `type.id=840` («Закрытые категории») и `direction.id=6078` («1,5 часовая игра юр лицо»), независимо от этого списка. Остальные направления типа `840` автоматически турнирами не становятся. Поиск дат включает `840`, затем события фильтруются по направлению; это может добавить загрузку дней с закрытыми событиями в пределах выбранного окна.
  - У импортированной связки `840/6078` устанавливается `isPublic=false`: скин по умолчанию закрытый, администратор может явно выбрать публикацию. События направления `6078` с типом `839` обрабатываются как обычные турниры. После установки обновления нужно штатно обновить нужные даты в ЦУП через «Обновить из Viva». Откат кода не удаляет сохранённый список: для исключения импортированных событий нужно обновить те же даты после отката; созданные скины сохраняются.
...
- `TOURNAMENTS_PUBLIC_AUTH_TIMEOUT_MS=5000` (опционально; timeout запросов auth)
