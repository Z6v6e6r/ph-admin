...
export class VivaTournamentsService {
  ...
  private readParticipantLevelLabel(
    ...
    const explicitLevel =
      this.readExplicitParticipantLevelLabel(subject) ??
      this.readExplicitParticipantLevelLabel(record);
    if (explicitLevel) {
  ...
  private readExplicitParticipantLevelLabel(record: VivaRawRecord | null): string | undefined {
    if (!record) {
      return undefined;
    }

    return this.readExplicitLevelLabelFromValue(record, new Set(), 0);
  }

  private readExplicitLevelLabelFromValue(
    value: unknown,
    ...
    if (Array.isArray(value)) {
      for (const item of value) {
        const label = this.readExplicitLevelLabelFromValue(item, visited, depth + 1);
        if (label) {
          return label;
        }
      }
    ...
    for (const key of directKeys) {
      const label = this.readNonNumericLevelLabel(record[key]);
      if (label) {
        return label;
      }
    }

    const structuredLabel = this.readStructuredLevelOptionLabel(record);
    if (structuredLabel) {
    ...
    for (const key of fieldArrayKeys) {
      const label = this.readExplicitLevelLabelFromFieldArray(record[key], visited, depth + 1);
      if (label) {
        return label;
      }
    }
    ...
    for (const key of nestedKeys) {
      const directLabel = this.readNonNumericLevelLabel(record[key]);
      if (directLabel) {
        return directLabel;
      }

      const nestedLabel = this.readExplicitLevelLabelFromValue(record[key], visited, depth + 1);
      if (nestedLabel) {
        return nestedLabel;
      }
    }
  ...
  private readExplicitLevelLabelFromFieldArray(
    value: unknown,
    visited: Set<VivaRawRecord>,
    depth: number
  ): string | undefined {
    ...
    for (const item of value) {
      ...
      const valueKeys = ['value', 'values', 'answer', 'content', 'grade', 'levelLabel', 'ratingLabel'];
  ...
  private readStructuredLevelOptionLabel(record: VivaRawRecord): string | undefined {
    const hasLevelValue = [
      'value',
      'values',
      'answer',
    ...
    const candidateKeys = ['label', 'title', 'name', 'code'];
    for (const key of candidateKeys) {
      const label = this.readNonNumericLevelLabel(record[key]);
      if (!label || /(level|rating|skill|уров|рейтинг|падел)/i.test(label)) {
        continue;
      }
      return label;
    }
  ...
  private readNonNumericLevelLabel(value: unknown): string | undefined {
    const label = this.readString(value);
    if (!label) {
      return undefined;
    }

    const normalized = label.replace(',', '.');
    return /^\d+(?:\.\d+)?$/.test(normalized) ? undefined : label;
  }
